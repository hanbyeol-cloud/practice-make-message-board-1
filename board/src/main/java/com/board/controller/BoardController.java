...
public class BoardController {
	...
	public void getListPage(Model model, @RequestParam("num") int num) throws Exception {
		...
		model.addAttribute("list", list);
		/*
		model.addAttribute("pageNum", page.getEndPageNum());
		...
		*/
		...
		/*
		 * // 게시물 총 개수 int count = service.count();
		 * 
		 * // 한 페이지에 출력할 게시물 개수 int postNum = 10;
		 * 
		 * // 하단 페이징 번호([ 게시물 총 개수 / 한 페이지에 출력할 개수 ]의 올림) int pageNum = (int)
		 * Math.ceil((double) count / postNum);
		 * 
		 * // 출력할 게시물 int displayPost = (num - 1) * postNum;
		 * 
		 * // 한번에 표시할 페이징 번호의 개수 int pageNum_cnt = 10;
		 * 
